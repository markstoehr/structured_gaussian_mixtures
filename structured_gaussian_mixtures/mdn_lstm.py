...
def load_data():
    ...
    Z = numpy.load(TRAIN)[:50000]
    Z_test = numpy.load(TEST)

    WINSIZE = Z.shape[1]//2
    X = Z[:,:WINSIZE].astype(dtype=numpy.float64)
    Y = Z[:,50:WINSIZE+50].astype(dtype=numpy.float64)

    X_test = Z_test[:,:WINSIZE].astype(dtype=numpy.float64)
    Y_test = Z_test[:,50:WINSIZE+50].astype(dtype=numpy.float64)

    X = [ numpy.lib.stride_tricks.as_strided(
        x, shape=(WINSIZE-50+1, 50), strides=(8, 8))
          for x in X]
    X_test = [ numpy.lib.stride_tricks.as_strided(
        x, shape=(WINSIZE-50+1, 50), strides=(8, 8))
               for x in X_test]
...
def init_params(options):
    ...
    # classifier
    params['U_sigma'] = 0.01 * numpy.random.randn(options['dim_proj'],
                                                  options['n_components']).astype(config.floatX)
    params['U_mu'] = 0.01 * numpy.random.randn(options['dim_proj'],
                                                  options['n_components']).astype(config.floatX)
    params['U_mix'] = 0.01 * numpy.random.randn(options['dim_proj'],
                                                  options['n_components']).astype(config.floatX)
    params['b_sigma'] = numpy.zeros((options['n_components'],)).astype(config.floatX)
    params['b_mu'] = numpy.zeros((options['n_components'],)).astype(config.floatX)
    params['b_mix'] = numpy.zeros((options['n_components'],)).astype(config.floatX)
...
def negative_log_likelihood(invsigma_given_x, mu, p_mix_given_x, y):
    ...
    return tensor.log(gauss_mix) + max_exponent
...
def build_model(tparams, options):
    ...
    invsigma_given_x, mu, p_mix_given_x = build_mdn_predict(proj, x, tparams)
    pred = [invsigma_given_x, mu, p_mix_given_x]

    # f_pred_prob = theano.function([x, mask], pred, name='f_pred_prob')
    # f_pred = theano.function([x, mask],
    #                          tensor.sum(mu * p_mix_given_x, axis=1),
    #                          name='f_pred')
    ...
    cost = -negative_log_likelihood(invsigma_given_x, mu, p_mix_given_x, y).mean()

    # return use_noise, x, mask, y, f_pred_prob, f_pred, cost
    return use_noise, x, mask, y, None, None, cost
...
def train_lstm(
    ...
    n_components=5,  # Vocabulary size
